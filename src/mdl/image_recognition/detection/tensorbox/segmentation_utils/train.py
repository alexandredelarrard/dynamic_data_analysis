...
def build_lstm_inner(H, lstm_input):
    ...
    build lstm decoder
    '''
    lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(H['lstm_size'], forget_bias=0.0, state_is_tuple=False)
    if H['num_lstm_layers'] > 1:
        lstm = tf.nn.rnn_cell.MultiRNNCell([lstm_cell] * H['num_lstm_layers'], state_is_tuple=False)
    else:
        lstm = lstm_cell
    ...
    with tf.variable_scope('decoder', reuse={'train': None, 'test': True}[phase]):
        ...
        if H['use_rezoom']:
            if H['rezoom_change_loss'] == 'center':
                ...
                inside = tf.reshape(tf.to_int64(tf.greater(iou, 0.5)), [-1])     ###### range >0.3 will keep boxes
            else:
...
#                    moving_avg.average(boxes_loss[p]))
        ...
        if phase == 'test':
            ...
            def log_image(np_img, np_confidences, np_boxes, np_global_step, pred_or_true):
                ...
                num_images = 20
                img_path = os.path.join(H['summary_dir'], '%s_%s.jpg' % ((np_global_step / H['logging']['display_iter']) % num_images, pred_or_true))
...
#            tf.summary.image(phase + '/pred_boxes', tf.pack(pred_log_img), max_outputs=10)
#            tf.summary.image(phase + '/true_boxes', tf.pack(true_log_img), max_outputs=10)
...
def train(H):
    ...
    '''
    ckpt_temporary_file = H['save_temporary_dir'].replace("//","/") + '/' + H['model_name']+'.ckpt'
    ckpt_final_file = H['save_dir'].replace("//","/") + '/' + H['model_name']+'.ckpt'

    with open(H['path_to_json']+'/hypes.json', 'w') as f:
    ...
    saver = tf.train.Saver(max_to_keep=1, write_version=tf.train.SaverDef.V1)
    writer = tf.summary.FileWriter(
        logdir=H['summary_dir'],
        flush_secs=10
    )
        ...
        for i in xrange(max_iter):
            ...
            if i % display_iter != 0:
                ...
                (train_loss, test_accuracy, 
                    _, _) = sess.run([loss['train'], accuracy['test'], train_op, smooth_op], feed_dict=lr_feed) #summary_op,
#                writer.add_summary(summary_str, global_step=global_step.eval())
            ...
            if global_step.eval() % H['logging']['save_iter'] == 0 and not (global_step.eval() == max_iter):
                saver.save(sess, ckpt_temporary_file, global_step=global_step)

            if global_step.eval() == max_iter:
                logging.error("save model of segmentation tensorbox")
                saver.save(sess, ckpt_final_file)

    logging.info("gettint the right treshold")
    tf.reset_default_graph()
    tf.get_default_session()
    h = find_th_tensorbox(H)

    return h


def find_th_tensorbox(H):

    """
    Function to  adjust the threshold used in the function add_rectangle in /utils/utils_tensorbox/utils/train_utils.py ; This threshold is used to determine the number of segmentation boxes selected based on their confidence. This function is called at the end of a "TRAIN" of the tensorbox segmentation model

    In order to adjust this threshold (and once the segmentation model has been trained):
    - run the segmentation model on some random images of the shelf database (we have duo image / csv associated )
    - compute the confidence associated to all of the boxes found by the segmentation model
    - Find the threshold that allow to take the mean right number of boxes
    - Store this threshold for TEST mode  and save it into hypes.json config file of segmentation model

    Inputs:
    - H hypes file used during the train phase
    - model_type :  either 'segmentation_model' or 'price_model' in order to pick the right model
    ...
    ### Find images used for train ####

    with open(H['data']['train_idl'], 'r') as f:
        data_in = json.load(f)

    dict_nb_box_image = {}
    for i in range(len(data_in)):
        dict_nb_box_image[data_in[i]['image_path']] = len(data_in[i]['rects'])

    ### Creation of Tf session
    ...
    print("\n=== [LOAD] ===\n")
    #========================================================================#
    #############   Load google graph and tensorflow for segmentation      #############
    H["batch_size"] = 1
    ...
    for path_im in dict_nb_box_image:
        image = Image.open(path_im)
        reference_data[path_im] = image
    ...
    with tf.Session(config = config) as sess:
        sess.run(tf.global_variables_initializer())
        tf.train.Saver().restore(sess, H['save_dir'] + '/segmentation_model.ckpt')

        for im_file in reference_data:

            # if product segmentation, no cropped performed on image.
            old_img = np.array(reference_data[im_file])
            new_img = imresize(old_img, ( H["image_height"], H["image_width"]), interp='cubic')

            # For each croped images
            # if price segmentation model, we need to crop image before processing it
            (np_pred_boxes, np_pred_confidences) = sess.run([pred_boxes, pred_confidences], feed_dict={x_in:new_img})

            pred_anno = al.Annotation()
            pred_anno.imageName = im_file

            min_conf = add_rectangles_test(H, [new_img], np_pred_confidences, np_pred_boxes, use_stitching=True, rnn_len=H['rnn_len'], min_conf=0.5, tau=0.25, nb_box_real = dict_nb_box_image[im_file])

            total_min_conf.append(min_conf)

    tf.reset_default_graph()
    tf.get_default_session()

    return np.mean(total_min_conf)


def add_rectangles_test(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0, show_removed=True, tau=0.25, nb_box_real = 0):
    ...
    all_conf = [rect.confidence for rect in pairs[0][0]]
    all_conf = sorted(all_conf)[::-1]

    if len(all_conf)> nb_box_real:
        min_conf= all_conf[nb_box_real-1]
    elif len(all_conf)>0 and len(all_conf)<=nb_box_real:
        min_conf= all_conf[-1]
    else:
        min_conf= 0
