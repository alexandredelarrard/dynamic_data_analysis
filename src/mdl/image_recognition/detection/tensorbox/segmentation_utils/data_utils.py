...
def annotation_jitter(I, a_in, min_box_width=20, jitter_scale_min=0.9, jitter_scale_max=1.1, jitter_offset=16, target_width=640, target_height=480):
    ...
    if i==0:
        I1 = exposure.adjust_gamma(I1, u)
    if i==1:
        I1 = exposure.adjust_log(I1, v)
    if i ==2:
        I1 = exposure.rescale_intensity(I1)
    if i ==3:
        I1 = exposure.adjust_sigmoid(I1)
    if i ==4:
        I1 = exposure.equalize_adapthist(I1, clip_limit=0.005, nbins=800, kernel_size=(20,20))
    ...
    x1 = int(max(0, px))
    y1 = int(max(0, py))
    x2 = int(min(rescaled_width, target_width - x1))
    y2 = int(min(rescaled_height, target_height - y1))
