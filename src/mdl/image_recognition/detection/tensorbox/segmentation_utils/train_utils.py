...
def generate_new_annos(H, annos):
    ...
    uu=0
    for anno in annos:
        #print 'new image %s' %uu
        uu=uu+1
        I = imread(anno.imageName) #anno['image']
        Iw_init = I.shape[1]
        Ih_init = I.shape[0]

        # check if I is the right size
        if I.shape[0] == Hh and I.shape[1] == Hw:
            new_annos.append({'image':I, 'anno':anno})
            continue

        # zoom I so that its shape is bigger than the standard size
        x_scale = Hw / float(I.shape[1])
        if x_scale > 1:
            I = imresize(I, (int(x_scale*I.shape[0]), int(x_scale*I.shape[1])))
        y_scale = Hh / float(I.shape[0])
        if y_scale > 1:
            I = imresize(I, (int(y_scale*I.shape[0]), int(y_scale*I.shape[1])))

        Iw = I.shape[1]
        Ih = I.shape[0]

        # reshape boxes after resize
        anno = rescale_boxes((Ih_init, Iw_init), anno, Ih, Iw)
        ...
        coords = list(itertools.product(coord_x, coord_y))  # [(0, 0), (0, Hh), (0, 2*Hh), ...(Hw, 0), (Hw, Hh), ...]
        cr_h=Hh
        cr_w=Hw

        crop_id = 0
        for coord in coords: # for each croped image
            #print 'new crop %s' %crop_id
            # creation of new anno
            anno_nw = al.Annotation()
            anno_nw.imageName = anno.imageName + "crop_%s" %crop_id #['anno']
            crop_id += 1

            # creation of sub_image
            sub_image = I[coord[1]:coord[1] + cr_h, coord[0]:coord[0]+cr_w]

            rects_nw = []
            for r in anno.rects: #['anno']

                left = max(r.x1, coord[0]);
                right = min(r.x2, coord[0]+cr_w);
                top = max(r.y1, coord[1]);
                bottom = min(r.y2, coord[1]+cr_h);

                # conditions:
                # - area > 0
                # - good disposition of coordinates
                # - box entirely in crop
                if left < right and top < bottom:

                   rect = al.AnnoRect()
                   rect.x1 = left - coord[0]
                   rect.y1 = top - coord[1]
                   rect.x2 = right - coord[0]
                   rect.y2 = bottom - coord[1]
                   rects_nw.append(rect)

            anno_nw.rects = rects_nw
            new_annos.append({'image':sub_image, 'anno':anno_nw})
...
def load_idl_tf(idlfile, H, jitter):
    ...
    logging.error("Anno taille : %s" %str(len(annos)))

    ######################################### new annos from cropped images
    cropped_annos = generate_new_annos(H, annos)
    logging.error('cropped_annos ' + str(len(cropped_annos)))

    if H['data']['truncate_data']:
        cropped_annos = cropped_annos[:15]

    compteur = 0
    for epoch in itertools.count():
        random.shuffle(cropped_annos)

        for modified_anno in cropped_annos:
                logging.error("cropped anno augmented: %i/%i" %(compteur, len(cropped_annos)))

                I = modified_anno['image']
                anno = modified_anno['anno']
...
def add_rectangles(H, orig_image, confidences, boxes, use_stitching=False, rnn_len=1, min_conf=0.1, show_removed=True, tau=0.25, nb_box_true = None):
