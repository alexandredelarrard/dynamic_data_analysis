...
def add_color_noise(im):
    u = np.random.uniform(0.7,1.3)
    v = np.random.uniform(0.6,1)
    i = np.random.randint(0,5)

    if i==0:
        im = exposure.adjust_gamma(im, u)
    if i==1:
        im = exposure.adjust_log(im, v)
    if i ==2:
        im = exposure.rescale_intensity(im)
    if i ==3:
        im = exposure.adjust_sigmoid(im)
    if i ==4:
        im = exposure.equalize_adapthist(im, clip_limit=0.005, nbins=800, kernel_size=(20,20))
...
def fast_warp(img, matrix, mode='edge', cval= 1.0):
        return tf._warps_cy._warp_fast(img, matrix.params,
                                              output_shape=(int(img.shape[0]), int(img.shape[1])), order=1, mode= mode, cval=cval)
...
def create_aug_matrices(nb_matrices, img_width_px, img_height_px,
    ...
    for _ in range(nb_matrices):
        ...
        liste_params.append([scale_x, rotation, shear, translation_x, translation_y])
        liste_matrix.append(matrix)

    return result, liste_params, liste_matrix

def apply_aug_matrices(images, matrices, transform_channels_equally=True,
                       channel_is_first_axis=False, random_order=True,
                       mode="edge", cval=0., interpolation_order=1,
                       seed=None, jitter = False, matrix_trans = 0):
    """Augment the given images usint()g the given augmentation matrices.
    ...
    #### check if pca on colors if proba_jitter > uniform random then pca is performed
    if jitter == True:
        proba_jitter=0.5
    elif isinstance(jitter, float):
        assert jitter >= 0.0 and jitter <= 1.0
        proba_jitter = jitter
    else :
        proba_jitter = 0
    ...
    for img_idx, image in enumerate(images):
        if apply_directly:
            ...
            matrix = matrices[img_idx]

            if int(image.shape[2])==4:
                im = tf.warp(image, matrix, mode="constant", cval=cval, order=interpolation_order)

                if np.random.uniform(0,1) < proba_jitter:
                    im = add_color_noise(im)

                result[img_idx, ...] = im

#                result[img_idx, ...] = np.dstack((np.uint8(im[:,:,0]*255),
#                                                      np.uint8(im[:,:,1]*255),
#                                                      np.uint8(im[:,:,2]*255),
#                                                      np.uint8(im[:,:,3]*255))).astype(np.uint8)
...
#                result[img_idx, ...] = np.dstack((np.uint8(im[:,:,0]*255),
#                                                      np.uint8(im[:,:,1]*255),
#                                                      np.uint8(im[:,:,2]*255))).astype(np.uint8)
...
class ImageAugmenter(object):
    ...
    def __init__(self, img_width_px, img_height_px, channel_is_first_axis=False,
                 ...
                 rotation_deg=0, shear_deg=0,
                 translation_x_px=0, translation_y_px=0,
                 transform_channels_equally=True, jitter= False):
        """
        Args:
    ...
    def pregenerate_matrices(self, nb_matrices, seed=None):
        ...
        if nb_matrices == 0:
            ...
            matrices, liste_params, matrix_trans = create_aug_matrices(nb_matrices,
                                           self.img_width_px,
                                           self.img_height_px,
    ...
    def augment_batch(self, images, seed=None):
        ...
        if self.pregenerated_matrices is not None:
            ...
            liste_params = self.liste_params
        else:
        ...
        # --------------------------------
        new_list =[ a_ + b_ for a_, b_ in zip(flipped_lr, liste_params)]
        return apply_aug_matrices(images, matrices,
                                  ...
                                  seed=seed, jitter = self.jitter, matrix_trans = matrix_trans) , new_list
    ...
    def plot_image(self, image, nb_repeat=40, show_plot=True):
        ...
        return self.augment_batch(images)
